...
def Woman_view(request):
    key = ImageModel.objects.filter(is_delete=False)
    return render(request,'products/woman.html',{'value':key})

def Man_view(request):
    key = ImageModel.objects.filter(is_delete=False)
    return render(request,'products/man.html',{'value':key})
...
def products(request):
    key = ImageModel.objects.filter(is_delete=False)
    return render(request,'adminpages/imglist.html',{'value':key})
